...
\begin{itemize}
    % \item \marginnote{10/27:}Let $\vec{\omega}$ point in the $\khat$ direction. Then
    % \begin{align*}
    %     \dv{\ihat}{t} &= \omega\jhat&
    %     \dv{\jhat}{t} &= -\omega\jhat&
    %     \dv{\khat}{t} &= 0
    % \end{align*}
    % \item Let $\dv*{\vec{a}}{t}$ denote the rate of change of $\vec{a}$ as measured by an inertial observer at rest relative to the origin, and let $\dot{\vec{a}}$ denote the rate of change of $\vec{a}$ as measured by an observer rotating with the solid body.
    % \begin{itemize}
    %     \item Then
    %     \begin{equation*}
    %         \dv{\vec{a}}{t} = \dot{a}+\vec{\omega}\times\vec{a}
    %     \end{equation*}
    % \end{itemize}
...
\subsection*{Section 5.4: Coriolis Force}
\begin{itemize}
